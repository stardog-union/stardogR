...
query <- function(stardog, q = "select (count(*) as ?n) {?s ?p ?o .}", namedGraph = NA, ...) {
  options <- list(...)
  if (!is.na(namedGraph)) {
    options <- c(options, `default-graph-uri` = namedGraph)
  }
  if (length(options) == 0) {
    query_list <- list(query = q)
  } else if (all(names(options) %in% c("limit", "reasoning",
                                       "useNamespaces", "schema",
                                       "offset", "default-graph-uri"))) {
  ...
  r <- GET(query_url, authenticate(stardog$username, stardog$password), accept("application/sparql-results+json"),
           query = query_list)
