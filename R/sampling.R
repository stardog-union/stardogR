...
#' @param namedGraph the named graph where the sample is saved.
#' @param ... The usual query parameters. For example, reasoning can be used.
#' @returns a data frame of the sampled triples.
#' @details
#' The sample is only saved if namedGraph is present. Logging adds some additional structure
...
#'
#' @export
sampleGraph <- function(stardog, triple, size = 100, namedGraph = NA, ...) {
  # if (logging && is.na(namedGraph)) stop(simpleError("Must supply a namedGraph if logging is turned on"))
  ...
  qs <- paste(service, '\n\n', 'select ', vars, ' where {\n',
             ...
             '\t\t', triple, ' .\n\t\t',
               '[] smp:size ', size, '\n\t\t}\n\t}', sep = "")
  df <- query(stardog, qs, namedGraph = namedGraph, ...)
  df
